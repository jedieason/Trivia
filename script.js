...
weeGPTButton.addEventListener('click', async () => {
    ...
    try {
        ...
        // 定義標籤內容
        const signature = '\n\n*Generated by WeeGPT*'; // 使用 Markdown 語法

        // 將標籤追加到詳解中
        const fullExplanation = explanation + signature;

        // 更新 currentQuestion 的 explanation
        currentQuestion.explanation = fullExplanation;

        // 更新頁面上的詳解區域
        document.getElementById('explanation-text').innerHTML = marked.parse(fullExplanation);
        document.getElementById('explanation').style.display = 'block';
